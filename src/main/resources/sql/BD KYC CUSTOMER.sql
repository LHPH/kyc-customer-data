...
BEGIN
    ...
    AND COALESCE(SECOND_NAME,'') = COALESCE(P_SECOND_NAME,'')
...
BEGIN
    ...
    ELSEIF (OPERATION = 0) THEN
        ...
        SELECT GET_ID_CUSTOMER(P_FIRST_NAME,P_SECOND_NAME,P_LAST_NAME,P_SECOND_LAST_NAME,P_RFC) INTO V_ID_CUSTOMER;
    ...
    IF (OPERATION = 2 OR OPERATION = 3)  AND
        NOT EXISTS(SELECT C.ID FROM CUSTOMER C WHERE C.ID=P_ID_CUSTOMER) THEN
    ...
    ELSIF OPERATION = 2 THEN
        ...
        UPDATE CUSTOMER SET ACTIVE='0' WHERE ID=V_ID_CUSTOMER;
        COMMIT;
        
    ELSIF OPERATION = 3 THEN
        
        DELETE FROM CUSTOMER_ADDRESS CA WHERE CA.ID_CUSTOMER=V_ID_CUSTOMER;
        DELETE FROM CUSTOMER WHERE ID = V_ID_CUSTOMER;
        COMMIT;
